(function () {
  ...
  function navigateToCandidates() {
    if (typeof window.navigateTo === "function") {
      window.navigateTo("candidati.html");
    } else {
      window.location.href = "candidati.html";
    }
  }
  ...
  function renderNotFound() {
    ...
    card.className = "glass-card p-8 rounded-3xl flex flex-col items-center text-center space-y-4";
    ...
    button.type = "button";
    button.className =
      "px-6 py-2 rounded-xl bg-[#1b4332] text-white text-sm font-semibold shadow hover:bg-[#1b4332]/90 transition";
  ...
  async function loadCandidateCore(candidateId) {
    ...
    try {
      ...
      if (window.IESupabase && window.IESupabase.showError) {
        window.IESupabase.showError(
          "Errore imprevisto nel caricamento del candidato.",
          "candidateProfile"
        );
      }
      return { candidate: null };
    ...
    if (editBtn) {
      editBtn.addEventListener("click", function () {
        ...
        if (window.IEPortal && window.IEPortal.links && window.IEPortal.links.candidateEdit) {
          ...
          target =
            "add-candidato.html?id=" + encodeURIComponent(candidateId) + "&mode=edit";
        }
        if (typeof window.navigateTo === "function") {
      ...
      rows.forEach(function (row) {
        ...
        if (offer && offer.id) {
          ...
          if (
            ...
            href =
              "add-offerta.html?id=" + encodeURIComponent(offer.id) + "&mode=view";
          }
  ...
  async function loadCandidateLogs(candidateId) {
    ...
    logs.forEach(function (log) {
      ...
      item.className = "border rounded-lg p-3 bg-gray-50";
      ...
      if (log && log.created_at) {
        try {
          ...
          if (!Number.isNaN(d.getTime())) {
            dateText = d.toLocaleString("it-IT");
          }
        } catch (_) {}
      }

      var message = (log && log.message) || "";
      var html = "";
      if (dateText) {
        html +=
          '<div class="text-xs text-gray-500 mb-1">' + escapeHtml(dateText) + "</div>";
      }
      html += '<div class="text-sm">' + escapeHtml(message) + "</div>";
