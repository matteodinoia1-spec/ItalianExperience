...
- On `DOMContentLoaded`, the app shell **awaits** `__IE_AUTH_GUARD__` before running `loadCurrentUserProfile()`, `initInactivityTimer()`, and the rest of bootstrap (header init, page initializers, data views, bottom-nav). So **no Supabase data loading runs before session validation** for pages that use this guard.
...
| **Defense in depth** | Secondary auth check (`await IEAuth.checkAuth()` / `requireAuth()`) added in: (1) `profile.js` at start of `requireAuthAndLoadProfile()`; (2) page-bootstrap runs after guard; data views run only after auth and profile (covers job-offers, clients, dashboard, candidates). Candidate detail page already had `requireAuth()` in `candidate.js` before loading data. |
